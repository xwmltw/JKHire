...
+ (void)showLoginVCWithBlock:(MKBlock)block{
    [self showLoginVCWithNeedUpDateEpInfo:NO block:block];
}
...
+ (void)showLoginVCWithNeedUpDateEpInfo:(BOOL)isUpdateEpInfo block:(MKBlock)block{
    ...
    loginVC.blcok = block;
    loginVC.isUpdateEpInfo = isUpdateEpInfo;
    ...
    //added by kizy : 考虑到当前视图有modal时无法再弹出新的modal,遍历所有VC直到找到没有Modal的VC
...
//    [[MKUIHelper getTopView] makeToast:msg];
//    [[self getCurrentRootViewController].view makeToast:msg];
...
+ (UIView *)noDataViewWithTitleArr:(NSArray *)noDataTipArr titleColor:(UIColor*)titleColor image:(NSString *)image button:(NSString *)btnTitle isAbsoluteWidth:(BOOL)isAbsoluteWidth isFullScreen:(BOOL)isFullScreen{
    if (!titleColor) {
    ...
    noDataImgView.tag = 110;
    [noDataView addSubview:noDataImgView];
    
    CGFloat sumHeight = noDataImgView.height;
    NSString *noDataTip;
    UILabel *tipLabel;
    CGFloat preHeight = 0;
    for (NSInteger index = 0; index < noDataTipArr.count; index ++) {
        ...
        CGFloat height = 0;
        if (isAbsoluteWidth) {
            height = [tipLabel contentSizeWithWidth:240].height;
            [tipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(noDataImgView.mas_bottom).offset(10 * (index + 1) + preHeight);
                make.centerX.equalTo(noDataView.mas_centerX);
                make.width.mas_lessThanOrEqualTo(@240);
            }];
        }else{
            height = [tipLabel contentSizeWithWidth:SCREEN_WIDTH - 64].height;
            [tipLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                make.top.equalTo(noDataImgView.mas_bottom).offset(10 * (index + 1) + preHeight);
                make.centerX.equalTo(noDataView.mas_centerX);
                make.left.equalTo(noDataView).offset(32);
                make.right.equalTo(noDataView).offset(-32);
            }];
        }
        preHeight += height;
    }
    
    sumHeight += (preHeight + 10 * noDataTipArr.count + 32);
...
+ (UIView *)noDataViewWithTitleArr:(NSArray *)noDataTipArr titleColor:(UIColor*)titleColor image:(NSString *)image button:(NSString *)btnTitle isAbsoluteWidth:(BOOL)isAbsoluteWidth{
    return [self noDataViewWithTitleArr:noDataTipArr titleColor:titleColor image:image button:btnTitle isAbsoluteWidth:isAbsoluteWidth isFullScreen:YES];
}
...
+ (UIView *)noDataViewWithTitleArr:(NSArray *)noDataTipArr titleColor:(UIColor*)titleColor image:(NSString *)image button:(NSString *)btnTitle{
    return [self noDataViewWithTitleArr:noDataTipArr titleColor:titleColor image:image button:btnTitle isAbsoluteWidth:YES];
}
