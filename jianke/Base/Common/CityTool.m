...
//VIP城市业务相关属性
static NSArray *s_filterAllVipCitys = nil; /*!< 开通VIP服务的所有城市 */
static NSArray *s_filterVipCityFirstLetters = nil; /*!< 开通VIP服务的所有城市首字母数组 */
static NSArray *s_allSortVIPCitys = nil; /*!< 排序好的所有开通VIP服务城市 */
...
/** VIP城市筛选 */
+ (void)getVIPCityArrayWithBlock:(MKBlock)block{
//    if (s_filterAllVipCitys) {
//        MKBlockExec(block, s_filterAllVipCitys);
//        return;
//    }
    [self getAllCityWithBlock:^(NSArray *allCity) {
        if (allCity.count) {
            //深复制 用于不改变s_AllCitys
            NSArray *allCitys = [[allCity mutableCopy] copy];
            
            //筛选VIP城市
            NSPredicate *preciate = [NSPredicate predicateWithFormat:@"enableVipService = 1"];
            NSArray *tmpArray = [allCitys filteredArrayUsingPredicate:preciate];
            s_filterAllVipCitys = tmpArray;
...
/** VIP城市首字母数组 */
+ (void)getVIPCityFirstLetterArrayWithBlock:(MKBlock)block{
    if (s_filterAllVipCitys.count) {
        s_filterVipCityFirstLetters = [self getCityFirstLetterArrayWithArray:s_filterAllVipCitys];
        MKBlockExec(block, s_filterVipCityFirstLetters);
    }
}

/** 按首字母排序好的所有开通VIP服务的城市 */
+ (void)getAllSortVIPCityWithBlock:(MKBlock)block{
//    if (s_allSortVIPCitys) {
//        MKBlockExec(block, s_allSortVIPCitys);
//        return;
//    }
    
    NSMutableArray *sortAllCity = [NSMutableArray array];
//    [self getVIPCityFirstLetterArrayWithBlock:^(NSArray *letterArray) {
        for (NSString *letter in s_filterVipCityFirstLetters) {
            NSArray *tmpArray = [s_filterAllVipCitys filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"pinyinFirstLetter like[cd] %@", letter]];
            [sortAllCity addObject:tmpArray];
        }
        s_allSortVIPCitys = sortAllCity;
        MKBlockExec(block, s_allSortVIPCitys);
//        return ;
//    }];
}
...
//    
    [self getAllCityWithBlock:^(NSArray *allCity) {
        // 获取字母数组
        s_cityFirstLetters = [self getCityFirstLetterArrayWithArray:allCity];
        MKBlockExec(block, s_cityFirstLetters);

    }];
}

+ (NSArray *)getCityFirstLetterArrayWithArray:(NSArray *)allCity{
...
/** 获取所有缓存城市 */
+ (void)getAllCityFromCacheWithBlock:(MKBlock)block{
    if (!s_allCitys) {
        [self getAllCityWithBlock:block];
        return;
    }
    MKBlockExec(block, s_allCitys);
}
