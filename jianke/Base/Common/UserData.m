...
- (BOOL)isHasAlertWithJobId:(NSString *)jobId{
     return [WDUserDefaults boolForKey:[NSString stringWithFormat:@"JobVasOrder%@", jobId]];
}
- (void)setHasAlertWithJobId:(NSString *)jobId{
    [WDUserDefaults setBool:YES forKey:[NSString stringWithFormat:@"JobVasOrder%@", jobId]];
    [WDUserDefaults synchronize];
}
...
- (BOOL)isHasOpenYetWithPostJob{
    return [WDUserDefaults boolForKey:[NSString stringWithFormat:@"isHasOpenYetWithPostJob"]];
}

- (void)setHasOpenYetWithPostJob{
    if (![self isHasOpenYetWithPostJob]) {
        [WDUserDefaults setBool:YES forKey:@"isHasOpenYetWithPostJob"];
        [WDUserDefaults synchronize];
    }
}
...
- (void)getUserStatus:(StatusBlock)statusBlock{
    if ([[UserData sharedInstance] isLogin]) {
        ...
        if (![[UserData sharedInstance] getIsLogoutActive]) {
            if (_loginFail) {
                MKBlockExec(statusBlock, UserLoginStatus_loginFail);
                return;
            }
...
- (void)userIsLoginWithNeedUpdateEpInfo:(BOOL)isUpdateEpInfo block:(MKBlock)block{
    if (_loginStatus) {
        ...
        [MKAlertView alertWithTitle:@"登录提示" message:@"您还没有登录唷~请登录解锁更多惊喜!" cancelButtonTitle:@"取消" confirmButtonTitle:@"确定" completion:^(UIAlertView *alertView, NSInteger buttonIndex) {
            if (buttonIndex == 1) {
                [UIHelper showLoginVCWithNeedUpDateEpInfo:isUpdateEpInfo block:block];
            }else{
...
- (void)userIsLogin:(MKBlock)block{
    [self userIsLoginWithNeedUpdateEpInfo:NO block:block];
}
...
- (void)userNameIsExact:(MKBlock)block{
    ...
    if (trueName.length) {
        MKBlockExec(block, trueName);
        return;
    }
    
    WEAKSELF
    MKAlertView *alertView = [[MKAlertView alloc] initWithTitle:@"完善姓名" message:@"请填写真实姓名" cancelButtonTitle:@"取消" confirmButtonTitle:@"确定" completion:^(UIAlertView *alertView, NSInteger buttonIndex) {
        if (buttonIndex == 1) {
            UITextField *nameTextField = [alertView textFieldAtIndex:0];
            if (!nameTextField.text || nameTextField.text.length < 2 || nameTextField.text.length > 5) {
                [UIHelper toast:@"请填写真实姓名"];
                [weakSelf userNameIsExact:block];
                return;
            }
            ...
            [[UserData sharedInstance] stuUpdateTrueName:nameTextField.text block:^(ResponseInfo *response) {
                if (response && response.success) {
                    ...
                    MKBlockExec(block, nameTextField.text);
                }
            }];
        }
    }];
...
- (BOOL)isEnablePersonalService{
    return ([UserData sharedInstance].city.enablePersonalService.integerValue == 1);
}

/*!< 当前城市是否开通团队服务 */
- (BOOL)isEnableTeamService{
    return ([UserData sharedInstance].city.enableTeamService.integerValue == 1);
}

/*!< 当前城市是否开通VIP服务 */
- (BOOL)isEnableVipService{
//    return ([UserData sharedInstance].city.enableVipService.integerValue == 1);
    return YES;
}

/*!< 当前是免审用户 */
- (BOOL)isJobExamVip{
    return ([[UserData sharedInstance] getEpModelFromHave].job_exam_vip.integerValue == 1);
}

/** 是否正在使用包代招 */
- (BOOL)isArrangedAgentEnabled{
    return (s_epModel.arranged_agent_vas_enable.integerValue == 1);
//    return YES;
}

/*! 当前账号是否是VIP账号 */
- (BOOL)isAccoutVip{
    return ([[UserData sharedInstance] getEpModelFromHave].account_vip_type.integerValue != 0);
}

/*!< 获取当前账号VIP图标 */
- (NSString *)getStatusImgWithAccout{
    return [[[UserData sharedInstance] getEpModelFromHave] getImgOfAccountVipType];
}
...
- (void)sethasShowPostJobGuide{
    [WDUserDefaults setBool:YES forKey:@"sbIsYanQingBiao"];
    [WDUserDefaults synchronize];
}

/**  设置是否首次进入 (发布引导) */
- (BOOL)isHasShowPostJobGuide{
    return [WDUserDefaults boolForKey:@"sbIsYanQingBiao"];
}

/**  设置是否首次进入 (发布引导) */
- (void)sethasShowJobManageGuide{
    [WDUserDefaults setBool:YES forKey:@"isHasShowJobManageGuide"];
    [WDUserDefaults synchronize];
}

/**  设置是否首次进入 (发布引导) */
- (BOOL)isHasShowJobManageGuide{
    return [WDUserDefaults boolForKey:@"isHasShowJobManageGuide"];
}
...
//    [self getJKModelWithAccountId:accountId readResumePage:nil block:block];
}
...
- (void)getJKModelWithAccountId:(NSString *)accountId ResumeId:(NSString *)resumeId readResumePage:(NSNumber *)readResumePage block:(MKBlock)block{
    NSString *content = [NSString stringWithFormat:@"\"read_resume_page\":\"%@\"",readResumePage];
    if (accountId) {
        content = [content stringByAppendingString:[NSString stringWithFormat:@",\"account_id\":\"%@\"", accountId]];
    }
    if (resumeId) {
        content = [content stringByAppendingString:[NSString stringWithFormat:@",\"resume_id\":\"%@\"", resumeId]];
    }
    content = [content stringByAppendingString:[NSString stringWithFormat:@",\"is_support_free_resume_num\":\"1\""]];
...
- (void)getJKModelWithAccountId:(NSString *)accountId ResumeId:(NSString *)resumeId readResumePage:(NSNumber *)readResumePage AppleyJobId:(NSNumber *)appleyJobId block:(MKBlock)block{
    ...
    if (readResumePage) {
        content = [content stringByAppendingString:[NSString stringWithFormat:@",\"read_resume_page\":\"%@\"", readResumePage]];
    }
    if (appleyJobId) {
        content = [content stringByAppendingString:[NSString stringWithFormat:@",\"apply_job_id\":\"%@\"", appleyJobId]];
    }
    [self getJKModelWithContent:content block:block];
...
- (void)getJKModelWithResumeId:(NSString *)resumeId block:(MKBlock)block{
    [self getJKModelWithResumeId:resumeId readResumePage:nil block:block];
}
...
- (void)getJKModelWithContent:(NSString *)content block:(MKBlock)block{
    RequestInfo* request = [[RequestInfo alloc] initWithService:@"shijianke_entGetResumeDetail" andContent:content];
...
- (void)getEPModelWithShowLoading:(BOOL)isShowLoading block:(MKBlock)block{
    ...
    //        block(s_epModel);
    //    } else {
    [self getEPModelWithContent:@"" isShowLoding:isShowLoading block:block];
    //    }
}

- (void)getEPModelIsNeedUpdate:(BOOL)isUpdate block:(MKBlock)block{
    if (isUpdate || !s_epModel) {
        [self getEPModelWithBlock:block];
    }else{
        MKBlockExec(block, s_epModel);
    }
}
...
//        block(s_epModel);
//    } else {
    [self getEPModelWithShowLoading:NO block:block];
//    }
}
...
- (void)getEPModelWithContent:(NSString *)content block:(MKBlock)block{
    [self getEPModelWithContent:content isShowLoding:NO block:block];
}

- (void)getEPModelWithContent:(NSString *)content isShowLoding:(BOOL)isShowLoding block:(MKBlock)block{
    RequestInfo *info = [[RequestInfo alloc] initWithService:@"shijianke_getEnterpriseBasicInfo" andContent:content];
    info.isShowLoading = isShowLoding;
...
- (void)getJobDetailWithJobId:(NSString *)jobId andIsFromSAB:(NSInteger)isFormSAB isShowLoding:(BOOL)isShowLoding Block:(MKBlock)block{
    ...
    NSString *content = [NSString stringWithFormat:@"job_id:%@,from_social_activist_broadcast:%@, account_type:1", jobId, [NSString stringWithFormat:@"%lu",(unsigned long)isFormSAB]];
...
- (void)getJobDetailWithJobUuid:(NSString *)jobUuid isShowLoding:(BOOL)isShowLoding block:(MKBlock)block{
    NSString *content = [NSString stringWithFormat:@"job_uuid:\"%@\", \"account_type\":\"1\"", jobUuid];
    NSString *pushId = [UserData sharedInstance].registrationID;
    if (pushId) {
        content = [content stringByAppendingFormat:@", \"push_id\":\"%@\"", pushId];
    }
...
- (void)entQueryApplyJobListWith:(GetQueryApplyJobModel *)paramModel isShowLoading:(BOOL)isShowLoading block:(MKBlock)block{
    ...
    NSString *content = [paramModel getContent];
    RequestInfo *request = [[RequestInfo alloc] initWithService:@"shijianke_entQueryApplyJobList" andContent:content];
    request.isShowLoading = isShowLoading;
    [request sendRequestWithResponseBlock:^(ResponseInfo *response) {
        if (response && response.success) {
        ...
        if (response && response.success) {
            ...
            [UIHelper showConfirmMsg:@"您已经用完免认证报名次数，实名认证可增加两次报名机会" title:@"权限不足" cancelButton:@"知道了" okButton:@"立即认证" completion:^(DLAVAlertView *alertView, NSInteger buttonIndex) {
                if (buttonIndex == 1) {
                    ...
                    MainNavigation_VC *nav = (MainNavigation_VC *)[MKUIHelper getCurrentRootViewController];
...
- (void)saveEPJobListWithArray:(NSArray*)list{
    NSArray* dicAry = [JobModel keyValuesArrayWithObjectArray:list];
    NSString* path = [WDCache getFullNameByFileName:@"home_ep_job_list" isPlist:NO];
    [dicAry writeToFile:path atomically:YES];
}
...
- (void)saveFindTalentListWithArray:(NSArray*)list{
    NSArray* dicAry = [JKModel keyValuesArrayWithObjectArray:list];
    NSString* path = [WDCache getFullNameByFileName:@"find_talent_list" isPlist:NO];
    [dicAry writeToFile:path atomically:YES];
}
...
- (NSArray *)getFindTalentList{
    ...
    NSArray* dicAry = [[NSArray alloc] initWithContentsOfFile:path];
    if (dicAry && dicAry.count) {
        return [JKModel objectArrayWithKeyValuesArray:dicAry];
    }
    return [[NSArray alloc] init];
}
...
- (void)handleGlobalRMUrlWithType:(GlobalRMUrlType)type block:(MKBlock)block{
    ...
    switch (type) {
        case GlobalRMUrlType_postServicePersonalJob:{
            if (!globalRM.wap_url_list || !globalRM.wap_url_list.post_service_personal_job_url.length) {
                return;
            }
            [self openWebViewWithUrl:globalRM.wap_url_list.post_service_personal_job_url block:block];
        }
            break;
        case GlobalRMUrlType_memberCenter:{
            if (!globalRM.wap_url_list || !globalRM.wap_url_list.jkjz_vip_center_url.length) {
                return;
            }
            [self openWebViewWithUrl:globalRM.wap_url_list.jkjz_vip_center_url block:block];
        }
            break;
        case GlobalRMUrlType_queryServicePersonalJobList:{
            if (!globalRM.wap_url_list || !globalRM.wap_url_list.query_service_personal_job_list_url.length) {
                return;
            }
            [self openWebViewWithUrl:globalRM.wap_url_list.query_service_personal_job_list_url block:block];
        }
            break;
        case GlobalRMUrlType_addServiceAgreement:{
            NSString *url = [NSString stringWithFormat:@"%@%@", URL_HttpServer, KUrl_toValueAddServiceAgreementPage];
            [self openWebViewWithUrl:url block:block];
        }
            break;
        default:
            break;
...
- (void)openWebViewWithUrl:(NSString *)url block:(MKBlock)block{
    if (!url.length) {
        return;
    }
