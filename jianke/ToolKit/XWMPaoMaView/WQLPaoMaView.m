...
//单次循环的时间
static const NSInteger animationDuration = 20;
...
- (instancetype)initWithFrame:(CGRect)frame withTitle:(NSMutableAttributedString *)title
{
    ...
    if (self) {
        ...
        myLable.attributedText = title;
        myLable.font = [UIFont systemFontOfSize:14.0f];
//        myLable.backgroundColor = [UIColor orangeColor];
        
        //计算文本的宽度
        NSString *textwidh = [NSString stringWithFormat:@"%@", title];
        CGFloat calcuWidth = [self widthForTextString:textwidh height:labelHeight fontSize:16.0f];
        ...
        if (calcuWidth>frame.size.width) {
            ...
            behindLabel.attributedText = title;
            behindLabel.font = [UIFont systemFontOfSize:14.0f];
