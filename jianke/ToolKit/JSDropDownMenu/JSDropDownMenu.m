...
//    CGSize textSize = [text sizeWithFont:[UIFont systemFontOfSize:14.0f] constrainedToSize:CGSizeMake(MAXFLOAT, 14)];
    ...
    if(_cellAccessoryView){
        _cellAccessoryView.frame = CGRectMake(_cellTextLabel.frame.origin.x+_cellTextLabel.frame.size.width+10, (self.frame.size.height-12)/2-2, 16, 12);
    }
}
...
- (UIColor *)textColor {
    if (!_textColor) {
        _textColor = MKCOLOR_RGB(0, 188, 212);
    }
    return _textColor;
}
...
- (UIColor *)textHightColor {
    if (!_textHightColor) {
        _textHightColor = MKCOLOR_RGB(0, 188, 212);
    }
...
- (void)reloadDataWith:(NSInteger)indexPath{
    self.leftSelectedRow = indexPath;
...
//        return [self.dataSource numberOfLeftSectionsInMenu:tableView];
//    }
//
//    return 0;
//}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [image setImage:[UIImage imageNamed:@"menu_location_bg"]];
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if (leftOrRight == 1) {
        ...
        if (indexPath.row == 0 && self.currentSelectedMenudIndex == 1) {
