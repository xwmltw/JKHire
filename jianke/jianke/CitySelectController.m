...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
    if (tableView == self.tableView) {
        switch (self.showType) {
            case CitySelectControllerShowType_default:{
                if (self.allSortCity) {
                    return self.allSortCity.count + 2;
                }
            }
                break;
            case CitySelectControllerShowType_filterVip:{
    ...
    if (tableView == self.tableView) {
        switch (self.showType) {
            case CitySelectControllerShowType_default:{
                ...
                break;
            case CitySelectControllerShowType_filterVip:{
                return [self.allSortCity[section] count];
            }
                break;
            default:{
                return [self.allSortCity[section] count];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.tableView) { // 当前tableView
        if (self.showType == CitySelectControllerShowType_default) {
            ...
            if (indexPath.row == 0) { // 显示字母
                [cell.cityFirstLetter setTitle:self.cityFirstLetterArray[indexPath.section] forState:UIControlStateNormal];
            } else {
                [cell.cityFirstLetter setTitle:nil forState:UIControlStateNormal];
            }
            [cell.cityFirstLetter setImage:nil forState:UIControlStateNormal];
            cell.cityName.text = [self.allSortCity[indexPath.section][indexPath.row] name];
        }
    ...
    if (!self.isShowSubArea) { // 不显示子区域
        ...
        if (tableView == self.tableView) { // 默认tableView
            if (self.showType == CitySelectControllerShowType_default) {
                ...
                CityModel *city = self.allSortCity[indexPath.section][indexPath.row];
                self.didSelectCompleteBlock(city);
            }
            
        } else { // 搜索的tableView
            CityModel *city = self.searchResuleArray[indexPath.row];
            MKBlockExec(self.didSelectCompleteBlock, city);
        }
    } else { // 需要显示子区域
...
- (void)getData{
    ...
    switch (self.showType) {
        ...
        case CitySelectControllerShowType_filterVip:{            
            // 所有城市
            [CityTool getVIPCityArrayWithBlock:^(NSArray *allCity) {
                weakSelf.allCity = allCity;
                //VIP城市首字母
                [CityTool getVIPCityFirstLetterArrayWithBlock:^(NSArray *cityFirstLetterArray) {
                    // 排序好的城市数组
                    weakSelf.cityFirstLetterArray = [NSKeyedUnarchiver unarchiveObjectWithData:[NSKeyedArchiver archivedDataWithRootObject:cityFirstLetterArray]];
                    [CityTool getAllSortVIPCityWithBlock:^(NSArray *allSortCity) {
                        weakSelf.allSortCity = allSortCity;
                        [weakSelf.tableView reloadData];
                    }];
                }];
