...
- (void)viewDidLoad {
    ...
    switch (self.postJobType) {
        case PostJobType_Personal:
            self.title = @"选择个人服务类型";
            break;
        case PostJobType_Team:{
            self.title = @"选择团队服务类型";
...
- (void)setDatasource{
    for (JobClassifyInfoModel* model in self.classifierArray) {
        ...
        }else if (self.postJobType == PostJobType_Personal){
            [_commonArray addObject:model];
        }else{
            [_commonArray addObject:model];
        }
...
- (void)getClassifyList{
    WEAKSELF
    [[XSJRequestHelper sharedInstance] getServiceClassifyInfoList:^(ResponseInfo *result) {
        if (result) {
            _commonArray = [[JobClassifyInfoModel objectArrayWithKeyValuesArray:[result.content objectForKey:@"service_classifier_list"]] mutableCopy];
            [_commonArray makeObjectsPerformSelector:@selector(convertTeamClassify)];
            [weakSelf.tableView reloadData];
        }
    }];
}
...
- (void)btnClick:(UIButton*)sender{
    ...
    [TalkingData trackEvent:[NSString stringWithFormat:@"选择包招岗位类型_%@",model.job_classfier_name]];
...
- (UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if ((section == 0 && (self.postJobType == PostJobType_common || self.postJobType == PostJobType_fast)) || self.postJobType == PostJobType_Personal || self.postJobType == PostJobType_Team || self.postJobType == PostJobType_Push) {
        return nil;
    }
