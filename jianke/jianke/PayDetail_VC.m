...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([model.payroll_check_status integerValue] == 1) {    //未验证
        if (model.is_cancel_pay_salary.integerValue != 1) {
            _model = model;
            [self showAlertView];
        }
    }
...
- (void)showAlertView{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"修改兼客信息" message:@"姓名或手机号填写有误，可重新修改，原先的工资不会被冒领，系统会将工资转移到修改后的人员账号中，您也可以在直接撤回工资到钱袋子" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"撤回工资", @"提交" ,nil];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (buttonIndex == 2){
        ...
        WEAKSELF
        [[XSJRequestHelper sharedInstance] entCancelUnConfirmStuSalaryWithItemId:_model.item_id block:^(id result) {
            if (result) {
                _model.is_cancel_pay_salary = @1;
                [weakSelf.tableView reloadData];
            }
        }];
    }
