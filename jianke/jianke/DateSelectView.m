...
- (void)calendar:(JTCalendarManager *)calendar prepareDayView:(JTCalendarDayView *)dayView{
    ...
    switch (self.type) {
        ...
        case DateViewType_makeCheck:{
            ...
            break;
        case DateViewType_PinganInsurance:{
            [self prepareForPinganInsuranceDateView:dayView];
        }
            break;
        case DateViewType_ViewPinganInsurance:{
            [self prepareForViewPinganInsuranceDateView:dayView];
        }
            break;
        default:
            break;
...
- (void)calendar:(JTCalendarManager *)calendar didTouchDayView:(JTCalendarDayView *)dayView{
    switch (self.type) {
        ...
        case DateViewType_makeCheck:{
            ...
            break;
        case DateViewType_PinganInsurance:{
            [self didTouchBuyInsuranceDayView:dayView];
        }
            break;
        default:
            break;
...
#pragma mark - ***** DateViewType_pinganInsurance ******
- (void)prepareForPinganInsuranceDateView:(JTCalendarDayView *)dayView{
    
    if([self isBetweenCanSelDateWithDayView:dayView]){ // 可投保日期
        
        if ([self isInDatesSelected:dayView.date]) { // 已选择
            dayView.textLabel.hidden = YES;
            dayView.imageView.image = [UIImage imageNamed:@"v230_payCheck"];
            dayView.imageView.hidden = NO;
            dayView.circleView.hidden = YES;
        }else if([self isPayDateWithDayView:dayView]){ // 历史投保过的记录
            dayView.textLabel.hidden = YES;
            dayView.imageView.hidden = NO;
            dayView.circleView.hidden = YES;
            dayView.imageView.image = [UIImage imageNamed:@"v231_check"];
...
- (void)prepareForViewPinganInsuranceDateView:(JTCalendarDayView *)dayView{
    if([self isPayDateWithDayView:dayView]){ // 历史投保过的记录
        dayView.textLabel.hidden = YES;
        dayView.imageView.hidden = NO;
        dayView.circleView.hidden = YES;
        dayView.imageView.image = [UIImage imageNamed:@"v231_check"];
...
#pragma mark - ***** 平安保险（与岗位无关） 点击处理 ******
- (void)didTouchBuyInsuranceDayView:(JTCalendarDayView *)dayView{
...
//                                dayView.imageView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 0.1, 0.1);
//                            } completion:nil];
        }else if ([self isPayDateWithDayView:dayView]){ //已购买的
