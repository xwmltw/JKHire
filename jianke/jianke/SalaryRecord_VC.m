...
- (void)viewDidLoad {
    ...
    if (self.isAddPay) {
        self.btntitles = @[@"添加发放人员"];
        [self initUIWithType:DisplayTypeTableViewAndBottom];
    }else{
        [self initUIWithType:DisplayTypeOnlyTableView];
    }
    [self initWithNoDataViewWithStr:@"无发放记录" onView:self.tableView];
...
- (void)headerRefresh{
    ...
    WEAKSELF
    [[XSJRequestHelper sharedInstance] queryAccDetail:self.queryParam moneyDetailType:@4 withJobId:self.jobId block:^(ResponseInfo *response) {
