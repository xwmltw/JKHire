...
- (void)setEpModel:(EPModel *)epModel{
    if (epModel) {
        ...
        self.authIconImg.hidden = epModel.id_card_verify_status.integerValue != 3;
    }
...
- (void)updateBtnStatusAtIndex:(NSInteger)index{
    switch (index) {
        case 0:{
            [self setBtnsSelect:self.indexBtn];
            [self showLineWithButton:self.indexBtn];
        }
            break;
        case 1:{
            [self setBtnsSelect:self.jobBtn];
            [self showLineWithButton:self.jobBtn];
        }
            break;
        case 2:{
            [self setBtnsSelect:self.Casebtn];
            [self showLineWithButton:self.Casebtn];
        }
            break;
        default:
            break;
