...
- (void)setModel:(PersonServiceModel *)model{
    ...
    self.labInvite.text = [NSString stringWithFormat:@"已邀约%@人", model.invite_num.description];
    self.labAccept.text = [NSString stringWithFormat:@"%@人接单", model.accept_invite_num.description];
    self.labPlatNum.text = [NSString stringWithFormat:@"平台推荐%@人", model.platform_invite_accept_num.description];
    if (model.service_type_classify_name && model.service_type_classify_name.length) {
        self.labServiceType.text = [NSString stringWithFormat:@"%@ - %@", [model getServiceTypeStr], model.service_type_classify_name];
    }else{
