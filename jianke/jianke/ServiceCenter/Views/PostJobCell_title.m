...
+ (instancetype)cellWithTableView:(UITableView *)tableView{
    ...
    if (!cell) {
        ...
        cell.tfTitle.borderStyle = UITextBorderStyleNone;
    }
    return cell;
}
...
- (void)refreshWithData:(JobModel*)model andIndexPath:(NSIndexPath *)indexPath{
    if (model) {
        ...
        if (model.job_title && model.job_title.length > 0) {
            ...
            self.labTitleWordNum.text = [NSString stringWithFormat:@"%lu/%lu", (unsigned long)self.tfTitle.text.length, (unsigned long)_maxLength];
        }
