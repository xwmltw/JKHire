...
- (void)setModel:(JKModel *)model isLookOther:(BOOL)isLookOther{
    ...
    if (model.age) {
        self.labAge.text = [NSString stringWithFormat:@"%@岁", model.age];
        [self.labArea mas_updateConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.labAge.mas_right).offset(25);
        }];
    }else{
        self.labAge.text = nil;
        [self.labArea mas_updateConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.labAge.mas_right).offset(0);
        }];
    }
