...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (![[XSJUserInfoData sharedInstance] getIsShowMyInfoTabBarSmallRedPoint]) {
        [self.tabBarController.tabBar hideSmallBadgeOnItemIndex:index];
    }
...
#pragma mark - ***** 按钮事件 ******
/** 充值 */
- (IBAction)btnPayMoneyOnClick:(UIButton *)sender {
    [TalkingData trackEvent:@"钱袋子_充值"];
    Recharge_VC* vc = [UIHelper getVCFromStoryboard:@"JK" identify:@"sid_mb_recharge"];
    [self.navigationController pushViewController:vc animated:YES];
}
...
//        [self.navigationController popViewControllerAnimated:YES];
    ...
    }else if(self.isIgnoreFromPay){
        for (UIViewController *controller in self.navigationController.viewControllers) {
            if ([controller isKindOfClass:[PersonSerMagCenter_VC class]] || [controller isKindOfClass:[SalaryJobGuide_VC class]] || [controller isKindOfClass:[SalaryRecord_VC class]]) {
                [self.navigationController popToViewController:controller animated:YES];
                break;
            }
        }
    }else if(self.isFromPaySalary){
        for (UIViewController *controller in self.navigationController.viewControllers) {
            if ([controller isKindOfClass:[SalaryRecord_VC class]]) {
                [self.navigationController popToViewController:controller animated:YES];
                return;
            }
        }
        MKBlockExec(self.block, self.jobId);
    }else if (self.isFromPromition){
        NSArray *childVCs = self.navigationController.childViewControllers;
        NSInteger count = childVCs.count;
        UIViewController *controller = nil;
        for (NSInteger index = (count - 1); index >= 0 ; index--) {
            controller = childVCs[index];
            if ([controller isKindOfClass:[JobDetail_VC class]] || [controller isKindOfClass:[JobMsgCenter_VC class]] || [controller isKindOfClass:[HiringJobList_VC class]] || [controller isKindOfClass:[PostJobSuccess_VC class]]) {
                [self.navigationController popToViewController:controller animated:YES];
                return;
            }
        }
...
//    for (UIViewController *controller in self.navigationController.viewControllers) {
//        if (![controller isKindOfClass:[InfoCenterController class]]) {
//            [self.navigationController popToViewController:controller animated:YES];
//        }
//    }
