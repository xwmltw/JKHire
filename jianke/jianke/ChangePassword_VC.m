...
@property (weak, nonatomic) IBOutlet UIImageView *imgSecret_1;
@property (weak, nonatomic) IBOutlet UIImageView *imgSecret_2;
@property (weak, nonatomic) IBOutlet UIImageView *imgSecret_3;
...
- (IBAction)oldPwd:(UITextField *)sender {
    if (sender.text.length > 0) {
        self.imgSecret_1.hidden = YES;
    }else{
        self.imgSecret_1.hidden = NO;
    }
    if (self.pwdAccountType == PwdAccountType_Login) {
...
- (IBAction)confirmPwd:(UITextField *)sender {
    if (sender.text.length > 0) {
        self.imgSecret_3.hidden = YES;
    }else{
        self.imgSecret_3.hidden = NO;
    }
    if (self.pwdAccountType == PwdAccountType_Login) {
        if (sender.text.length > 32) {
            sender.text = [sender.text substringToIndex:32];
