...
- (void)checkJkInfo{
    if (self.targetInfo.resumeId.integerValue) {
        ...
        vc.isFromToLookUpResume = 5;
        vc.resumeId = self.targetInfo.resumeId;
        [self.navigationController pushViewController:vc animated:YES];;
    }else if (self.resumeId){
        LookupResume_VC *vc = [[LookupResume_VC alloc] init];
        vc.isFromToLookUpResume = 5;
        vc.resumeId = (NSNumber *)self.resumeId;
        [self.navigationController pushViewController:vc animated:YES];;
    }
}
...
//            [UIHelper openInsterJobVCWithRootVC:self];
        ...
        case WdSystemNoticeType_RedirectJobVasPage:{        // 92:跳转客户端付费推广页面
            ...
            break;
        case WdSystemNoticeType_BuyInsuranceFail:{      // 94:兼职保险失败
            MyInsuranceHistory_vc *viewCtrl = [[MyInsuranceHistory_vc alloc] init];
            viewCtrl.insurance_record_id = sysMsg.insurance_record_id;
            [self.navigationController pushViewController:viewCtrl animated:YES];
        }
            break;
        case WdSystemNoticeType_StuAgreeServicePersonalInvite:      // 90:兼客同意个人服务邀约
        case WdSystemNoticeType_PersonalServiceOrderBackouted:{       // 96:个人服务需求被下架
            WebView_VC *viewCtrl = [[WebView_VC alloc] init];
            viewCtrl.url = [NSString stringWithFormat:@"%@%@?job_id=%@", URL_HttpServer, KUrl_toServicePersonalJobApplyListPage, sysMsg.service_personal_job_id];
            [self.navigationController pushViewController:viewCtrl animated:YES];
        }
            break;
        case WdSystemNoticeType_RedirectJobRecuitJobNumPage:{       // 96:跳转【在招岗位数】页面
            HiringJobList_VC *viewCtrl = [[HiringJobList_VC alloc] init];
            viewCtrl.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:viewCtrl animated:YES];
        }
            break;
        case WdSystemNoticeType_RedirectJobRefreshVasSpreadPage:{   // 97:跳转【刷新推广】页面
            JobAutoRefresh_VC *viewCtrl = [[JobAutoRefresh_VC alloc] init];
            viewCtrl.hidesBottomBarWhenPushed = YES;
            [self.navigationController pushViewController:viewCtrl animated:YES];
        }
            break;
        case WdSystemNoticeType_RdirectAccountVipRechargePage:{ //99:跳转会员购买页面
            VipIPacket_VC *vc = [[VipIPacket_VC alloc] init];
            vc.hidesBottomBarWhenPushed = YES;
            WEAKSELF
            vc.block = ^(id result){
                [weakSelf.navigationController popToViewController:weakSelf animated:NO];
                VipInfoCenter_VC *viewCtrl = [[VipInfoCenter_VC alloc] init];
                [self.navigationController pushViewController:viewCtrl animated:YES];
            };
            [self.navigationController pushViewController:vc animated:YES];
        }
            break;
        case REDIRECT_ACCOUNT_RESUME_RECHARGE_PAGE:{ //100：跳转简历数购买页
