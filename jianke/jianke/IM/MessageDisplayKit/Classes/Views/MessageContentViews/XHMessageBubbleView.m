            ...
            if (message.messageMediaType == XHBubbleMessageMediaTypeSystem) {
                ...
                if (sysMsg) {
                    switch (sysMsg.notice_type.intValue) {
                        ...
                        case WdSystemNoticeType_JobVerifyFail:              //岗位审核未通过
                        ...
                        case WdSystemNoticeType_JopAnswerQuestion:          //关于岗位的提问和答复
    ...
    if (message.messageMediaType == XHBubbleMessageMediaTypeSystem) {
        ...
        if (sysMsg) {
            switch (sysMsg.notice_type.intValue) {
                ...
                case WdSystemNoticeType_ServicePersonalAuditFail:   //个人服务申请审核不通过
                ...
                case WdSystemNoticeType_RedirectJobRefreshVasSpreadPage:   // 97:跳转【刷新推广】页面
                case WdSystemNoticeType_RdirectAccountVipRechargePage://99:跳转会员购买页面
                case REDIRECT_ACCOUNT_RESUME_RECHARGE_PAGE:{//100:跳转购买简历数页
                    bUnderLing = NO;
...
//            timeLabel.textColor = [UIColor whiteColor];
        ...
        if (!_lookupDetailBtn) {
            ...
            [lookupDetailBtn setImage:[UIImage imageNamed:@"job_icon_push"] forState:UIControlStateNormal];
