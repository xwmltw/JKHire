...
@property (weak, nonatomic) IBOutlet UITextField *tfPwdCheck;
...
@property (weak, nonatomic) IBOutlet UIImageView *imgSecret_2;
...
- (void)viewDidLoad {
    ...
    if (self.pwdAccountType == PwdAccountType_Login) {
        ...
        self.tfPwdCheck.keyboardType = UIKeyboardTypeDefault;
    }else if (self.pwdAccountType == PwdAccountType_MoneyBag){
        self.title = @"找回钱袋子密码";
        self.tfPwd.keyboardType = UIKeyboardTypeNumberPad;
        self.tfPwdCheck.keyboardType = UIKeyboardTypeNumberPad;
...
- (IBAction)btnUpdatePwdOnClick:(UIButton *)sender {
    ...
    if (![self.tfPwd.text isEqualToString:self.tfPwdCheck.text]){
        [UIHelper toast:@"两次输入的密码不一致"];
        return;
    }
...
- (IBAction)bgViewOnClick:(id)sender {
    ...
    [self.tfPwdCheck resignFirstResponder];
}
//发送请求
...
- (IBAction)confirmNum:(UITextField *)sender {
    if (sender == self.tfPwdCheck) {
        if (self.tfPwdCheck.text.length > 0) {
            self.imgSecret_2.hidden = YES;
        }else{
            self.imgSecret_2.hidden = NO;
        }
    }
