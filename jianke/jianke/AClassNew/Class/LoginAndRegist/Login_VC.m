...
@property (nonatomic, assign) BOOL isFromRegistRequest; /*! 是否是来自注册页面的请求登录 */
...
- (void)viewDidLoad {
    ...
    if (self.isToRegister) {
        ...
        WEAKSELF
        vc.block = ^(id result){
            weakSelf.isFromRegistRequest = YES;
            ...
            [weakSelf sendLoginRequestWithUsername:username password:password type:LoginPwdType_commonPassword];
        };
...
- (void)setLoginPwdType{
    self.imgPwd.hidden = !_loginPwdCommonType;
...
- (void)sendLoginRequestWithUsername:(NSString *)username password:(NSString *)password type:(LoginPwdType)type{
    WEAKSELF
    [[XSJRequestHelper sharedInstance] loginWithUsername:username pwd:password loginPwdType:type isShowNetworkErr:YES isUpdateEpInfo:self.isUpdateEpInfo bolck:^(ResponseInfo *result) {
        if (result) {
            if (type == LoginPwdType_dynamicSmsCode && result.errCode.integerValue == 8) {
                ...
                [weakSelf lastLoginSuccess:result];
            }
        }
    }];
}
...
- (void)reLogin{
    NSString *username = [[XSJUserInfoData sharedInstance] getUserInfo].userPhone;
    NSString *password = [[XSJUserInfoData sharedInstance] getUserInfo].password;
    [self sendLoginRequestWithUsername:username password:password type:LoginPwdType_commonPassword];
}

- (void)lastLoginSuccess:(ResponseInfo *)response{
    if (self.isFromNewFrature) {
        if (self.isFromRegistRequest) {
