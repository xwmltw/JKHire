...
+ (void)switchIsToEP:(BOOL)isToEP{
    ...
    [UIApplication sharedApplication].keyWindow.rootViewController = [[MainTabBarCtlMgr sharedInstance] creatEPTabbar];
}
...
+ (void)showToast:(NSString*)message{
    [[MKUIHelper getCurrentRootViewController].view makeToast:message];
}
...
- (void)showCommentAlertWithVC:(UIViewController*)rootVc{
    ...
    if (showTime == 4) {     //已经评价过
        ...
        DLAVAlertView* alertView = [[DLAVAlertView alloc] initWithTitle:@"意见反馈" message:@"新版兼客优聘体验如何?\n我们十分在意您的反馈，告诉我们吧！\n感谢您的支持。" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"我要吐槽", @"以后再说",nil];
