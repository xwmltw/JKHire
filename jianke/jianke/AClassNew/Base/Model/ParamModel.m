...
@implementation InsurancePolicyParamModel


- (void)setValueWithDic:(NSDictionary *)resultDic{
    if (resultDic == nil) {
        return;
    }
    self.insured_telephone = [resultDic objectForKey:@"telphone"];
    self.insurance_date_list = [resultDic objectForKey:@"apply_date_list"];
    self.insured_true_name = [resultDic objectForKey:@"true_name"];
    self.insured_id_card_num = [resultDic objectForKey:@"id_card_no"];
}
...
- (NSDictionary *)objectClassInArray{
    return @{@"insurance_policy_list" : [InsurancePolicyParamModel class]};
}

@end

@implementation RechargeRecruitParam
@end

@implementation QueryTalentParam
@end
