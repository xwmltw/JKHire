...
- (void)initWithNoDataViewWithStr:(NSString *)str labColor:(UIColor*)labColor imgName:(NSString*)imgName button:(NSString *)btnTitle onView:(UIView *)view{
    ...
    if (_viewWithNoData){
        [_viewWithNoData.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [obj removeFromSuperview];
        }];
        [_viewWithNoData removeFromSuperview];
        _viewWithNoData = nil;
    }
...
- (UIViewController *)popBackToView{
    if (self.navigationController) {
        UIViewController *viewCtrl1 = [UserData sharedInstance].popViewCtrl;
        UIViewController *viewCtrl2 = [UserData sharedInstance].popManageViewCtrl;
        if ([self canGoBack:viewCtrl1]) {
            [self.navigationController popToViewController:viewCtrl1 animated:YES];
            return viewCtrl1;
        }else if ([self canGoBack:viewCtrl2]){
            [self.navigationController popToViewController:viewCtrl2 animated:YES];
            return viewCtrl2;
        }
    }
    return nil;
...
- (BOOL)canGoBack:(UIViewController *)viewCtrl{
    return [self hasViewCtrl:viewCtrl] && self != viewCtrl;
}

- (BOOL)hasViewCtrl:(UIViewController *)viewCtrl{
    if (self.navigationController) {
       return [self.navigationController.viewControllers containsObject:viewCtrl];
    }
    return NO;
}
