...
@interface JiankeCollection_VC () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, JKMangeHeaderViewDalegate, JKMangeFooterViewDelegate>{
...
- (void)viewDidLoad {
    ...
    self.dataSource = @[@(JKBannerType_Manage), @(JKBannerType_Salary), @(JKBannerType_Record), @(JKBannerType_JianLi), @(JKBannerType_IM),@(JKBannerType_NotifRecord),  @(JKBannerType_Add), @(JKBannerType_OutPut), @(JKBannerType_outSource)];
    [self initUI];
}
...
- (void)initUI{
    ...
    collectionView.backgroundColor = [UIColor XSJColor_newWhite];
    ...
    self.layout.itemSize = CGSizeMake((SCREEN_WIDTH - 24 - 12) / 3, 126);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (type) {
        ...
        case JKBannerType_jobVas:{
            ...
            cell.subTitleLab.text = @"曝光量提升2-5倍";
        }
            break;
        case JKBannerType_outSource:{
            cell.titleLab.text = @"服务外包";
            cell.img.image = [UIImage imageNamed:@"v3_epgl_icon_fwwb"];
        }
            break;
        default:
            break;
...
#pragma mark - JKMangeFooterViewDelegate
...
- (void)showHireSheet{
    ...
    [MKActionSheet sheetWithTitle:nil buttonTitleArray:@[@"手动补录",@"链接补录"] block:^(MKActionSheet *actionSheet, NSInteger buttonIndex) {
        if (buttonIndex == 0) {         //手动
            ...
            vc.title = @"手动补录";
            vc.fromViewType = FromViewType_ManualAddJK;
            vc.job_id = _jobId;
...
- (void)pushJKmange{
    ...
    emVC.managerType = ManagerTypeEP;
    emVC.isAccurateJob = @1;
...
- (void)payJobVasOrder{
    ...
    viewCtrl.jobId = self.jobId;
    if (_JobDetailModel.parttime_job.status.integerValue == 3 || self.jobModel.status.integerValue == 3) {
