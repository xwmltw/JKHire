...
@interface JKMange_HeaderView ()

@property (weak, nonatomic)  UILabel *viewNumLab;
@property (weak, nonatomic)  UILabel *accNumLab;
...
@implementation JKMange_HeaderView
...
- (void)setupViews{
    ...
    UILabel *accNumLab = [UILabel labelWithText:@"" textColor:[UIColor XSJColor_tGrayDeepTinge] fontSize:32.0f];
    accNumLab.textAlignment = NSTextAlignmentCenter;
    _accNumLab = accNumLab;
    UILabel *labAccNumDes = [UILabel labelWithText:@"报名人数" textColor:[UIColor XSJColor_tGrayDeepTinge] fontSize:14.0f];
    ...
    [self addSubview:accNumLab];
    [self addSubview:labAccNumDes];
    ...
    [viewNumLab mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.equalTo(accNumLab);
    }];
    
    [accNumLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(labBaoguangNum);
        make.left.equalTo(viewNumLab.mas_right);
    ...
    [labAccNumDes mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(labViewNumDes);
        make.centerX.equalTo(accNumLab);
    }];
...
- (void)btnOnClick:(UIButton *)sender{
    if ([self.delegate respondsToSelector:@selector(JKMange_HeaderView:)]) {
        [self.delegate JKMange_HeaderView:self];
    }
}
