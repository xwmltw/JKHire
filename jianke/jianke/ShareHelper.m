...
//    UIImage *image = nil;
    if (info.share_img_url && info.share_img_url.length) {
        NSData *data = [NSData dataWithContentsOfURL:[[NSURL alloc] initWithString:info.share_img_url]];
        [[UMSocialDataService defaultDataService] postSNSWithTypes:@[snsPlatform] content:info.share_content image:data location:nil urlResource:nil presentedController:vc completion:^(UMSocialResponseEntity *response){
        ...
        [[UMSocialDataService defaultDataService] postSNSWithTypes:@[snsPlatform] content:info.share_content image:[UIImage imageNamed:@"main_icon_logo"] location:nil urlResource:nil presentedController:vc completion:^(UMSocialResponseEntity *response){
            ELog(@"UM response %u",response.responseCode);
            if (response.responseCode == UMSResponseCodeSuccess) {
                [UIHelper toast:@"分享成功"];
            }
...
//    if ([snsPlatform isEqualToString:UMShareToQQ] || [snsPlatform isEqualToString:UMShareToQzone]) {    //QQ或者空间 用url图片好像会分享失败
//        urlResource = nil;
//    }
