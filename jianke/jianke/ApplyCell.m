...
@property (weak, nonatomic) IBOutlet UIView *contantViewTop;
@property (weak, nonatomic) IBOutlet UIView *contantViewBot;
@property (weak, nonatomic) IBOutlet UIView *contantViewSub;
@property (weak, nonatomic) IBOutlet UIImageView *imgHeader;
@property (weak, nonatomic) IBOutlet UILabel *labName1;
@property (weak, nonatomic) IBOutlet UILabel *labDate1;
@property (weak, nonatomic) IBOutlet UIButton *btnCall;
- (IBAction)btnCallOnClick:(UIButton *)sender;
...
@property (weak, nonatomic) IBOutlet UILabel *labBreak;
...
- (void)setCellType:(ApplyCellType)cellType{
    _cellType = cellType;
    self.contantViewTop.hidden = NO;
    self.contantViewBot.hidden = NO;
    self.contantViewSub.hidden = YES;
    self.adjustDateBtn.hidden = NO;
    self.adjustDateBtn.hidden = NO;
    switch (cellType) {
        case ApplyCellType_jobManageForNormal:{
            self.adjustDateBtn.hidden = YES;
        }
            break;
        case ApplyCellType_jobManageForHire:{
            self.contantViewSub.hidden = NO;
            self.contantViewTop.hidden = YES;
            self.contantViewBot.hidden = YES;
            self.labDate1.hidden = NO;
            self.labBreak.hidden = YES;
        }
            break;
        case ApplyCellType_jobManageForRejected:{
            self.contantViewSub.hidden = NO;
            self.contantViewTop.hidden = YES;
            self.contantViewBot.hidden = YES;
            self.labDate1.hidden = YES;
            self.labBreak.hidden = NO;
        }
            break;
        default:
            break;
...
- (void)setCellModel:(JKModel *)cellModel{
    _cellModel = cellModel;
    JKModel *jkModel = cellModel;
    ...
    [self.imgHeader setCornerValue:2.0f];
    [self.imgHeader sd_setImageWithURL:[NSURL URLWithString:jkModel.user_profile_url] placeholderImage:[UIImage imageNamed:@"main_icon_avatar"]];
    ...
    self.labName1.text = jkModel.true_name;
    ...
    self.labDate1.text = self.dateLabel.text;
    
    // 学校
    if (jkModel.school_name.length) {
        self.schoolLabel.hidden = NO;
        self.schoolLabel.text = jkModel.school_name;
    }else{
        self.schoolLabel.hidden = YES;
    }
    
    // 放鸽子 & 完工 次数
    self.breakCountLabel.text = [NSString stringWithFormat:@"%@ 次完工  |  %@ 次放鸽子", jkModel.work_experice_count.description, jkModel.break_promise_count.description];
    self.labBreak.text = self.breakCountLabel.text;
...
- (NSArray *)sortForWorkDate{
    NSArray *tmpWorkTime = [self.cellModel.stu_work_time sortedArrayUsingComparator:^NSComparisonResult(NSNumber *obj1, NSNumber *obj2) {
...
- (void)callBack:(ActionType)actionType{
    if ([self.delegate respondsToSelector:@selector(applyCell:cellModel:actionType:)]) {
...
//        NSArray *tmpArray = [dateStr componentsSeparatedByString:@","];
        ...
        [weakSelf.dateAlertView showWithCompletion:^(DLAVAlertView *alertView, NSInteger buttonIndex) {
            ...
            if (buttonIndex == 1) {
                ...
                // 调整发送请求
                [[UserData sharedInstance] entChangeStuOnBoardDateWithApplyJobId:self.cellModel.apply_job_id dayArray:applyDateStrArrayNew block:^(ResponseInfo *response) {
                    ...
                    weakSelf.cellModel.stu_work_time = applyDateStrArray;
...
- (IBAction)phoneBtnClick:(UIButton *)sender{
    ...
    if ([[UserData sharedInstance] getLoginType].integerValue == 1 && [[UserData sharedInstance] getEpModelFromHave].identity_mark.integerValue == 2) {
        [[XSJRequestHelper sharedInstance] callFreePhoneWithCalled:self.cellModel.contact.phone_num block:nil];
    }else{
        [[MKOpenUrlHelper sharedInstance] callWithPhone:self.cellModel.contact.phone_num];
    }
...
- (void)noUseWithReason:(NSString *)reason{
    [TalkingData trackEvent:@"兼客管理_已报名_不合格"];
    WEAKSELF
    [[UserData sharedInstance] entEmployApplyJobWithWithApplyJobIdList:@[self.cellModel.apply_job_id] jobId:nil employStatus:@(2) employMemo:reason block:^(ResponseInfo *response) {
        weakSelf.cellModel.trade_loop_status = @(3);
        weakSelf.cellModel.trade_loop_finish_type = @(2);
//        [weakSelf reflushCell];
...
- (IBAction)applyBtnClick:(UIButton *)sender{
    ...
    [self hideRedPoint];
    WEAKSELF
    [[UserData sharedInstance] entEmployApplyJobWithWithApplyJobIdList:@[self.cellModel.apply_job_id] jobId:nil employStatus:@(1) employMemo:nil block:^(ResponseInfo *response) {
        weakSelf.cellModel.trade_loop_status = @(2);
//        [weakSelf reflushCell];
...
- (void)hideRedPoint{
    self.cellModel.ent_big_red_point_status = @(0);
    [[UserData sharedInstance] entReadApplyJobResumeWithApplyJobId:self.cellModel.apply_job_id.stringValue block:nil];
}
