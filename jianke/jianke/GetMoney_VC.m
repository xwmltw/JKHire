...
- (void)viewDidLoad {
    ...
    [[XSJRequestHelper sharedInstance] getClientGlobalInfoWithBlock:^(ClientGlobalInfoRM *globalModel) {
        if (globalModel) {
            weakSelf.labAlipayDesc.text = globalModel.alipay_withdraw_desc_youpin_v1;
        }
    }];
}
...
- (IBAction)btnAlipayOnclick:(UIButton *)sender {
    ...
    [[XSJRequestHelper sharedInstance] getClientGlobalInfoWithBlock:^(ClientGlobalInfoRM *globalModel) {
        if (globalModel) {
            [XSJUIHelper showAlertWithTitle:nil message:globalModel.alipay_withdraw_desc_youpin_v1 okBtnTitle:@"我知道了"];
        }
    }];
}
