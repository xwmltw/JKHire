...
//@property (nonatomic, copy) NSString *working_area;         /*!< 区域 自定义字段 */
//@property (nonatomic, copy) NSString *working_address;  /*!< 详细地址 自定义字段 */
//==============自定义部分  勿删==============
...
@property (nonatomic, copy) NSNumber* job_type;             /*!< 岗位类型，1表示普通岗位，2表示抢单岗位，3代发岗位，4.快速发布岗位 5.限制岗位 如果没传这个字段，默认是1 */
...
@property (nonatomic, copy) NSNumber* is_left_job_vas_intelligent_refresh;/*！< 是否处于刷新状态 1是 0否*/
...
@property (nonatomic, copy) NSNumber* status;                       /*!< 岗位状态  0待审核 1审核中, 2已发布 , 3已结束 */
...
@property (nonatomic, copy) NSNumber *is_show_vas_count;    /*!< 是否显示推广效果值 */
@property (nonatomic, copy) NSNumber *job_vas_view_count;   /*!< 推广带来的浏览效果值 */
@property (nonatomic, copy) NSString *job_vas_effect_str;   /*!< 推广带来的浏览效果（包括文字） */
@property (nonatomic, copy) NSNumber *job_vas_exposure_num; /*!< 推广期间新增加的曝光量 */
@property (nonatomic, copy) NSNumber *exposure_count;   /*!< 曝光量 */

// v2.4
@property (nonatomic, assign, getter=isReaded) BOOL readed; /*!< 是否已读, 本地字段 */

// added by kizy V3.1.7
@property (nonatomic, copy) NSNumber *is_vip_job;   /*!< 是否是vip岗位 1：是 0:否 */

// added by kizy V3.1.8
@property (nonatomic, copy) NSNumber *guarantee_amount; /*!< 保证金金额 单位为元 */
@property (nonatomic, copy) NSNumber *guarantee_amount_status;  /*!< 保证金状态：0 未缴纳 1已缴纳 2 被冻结 3 退回中 4 已退回 */
@property (nonatomic, copy) NSNumber *guarantee_amount_refund_time; /*!< 保证金退款时间 */
@property (nonatomic, copy) NSNumber *guarante_amount_pay_money;    /*!< 需要缴纳的保证金金额,单位是分 */

// added by kizy v3.2.1
@property (nonatomic, copy) NSNumber *is_fit_job_limit; /*!< 是否符合岗位限制 1是 0否 */
@property (nonatomic, copy) NSString *fit_job_limit_desc;   /*!< 是否岗位限制结果描述 */
@property (nonatomic, copy) NSString *job_last_vas_push_str;    /*!< 岗位最近一次推送推广信息展示字符 */
@property (nonatomic, copy) NSNumber *job_post_ent_verify_status;   /*!< 岗位发布时雇主的认证状态 */


//个人服务需求(要用)******兼客优聘
@property (nonatomic, copy) NSNumber *service_personal_job_id;  // 个人服务需求id
@property (nonatomic, copy) NSString *service_title;    // 服务名称
@property (nonatomic, copy) NSNumber *service_type;  // 服务类型  1：模特 2：礼仪3：主持  4：商演 5：家教 6：校园代理 7:促销人员
...
@property (nonatomic, copy) NSNumber *is_invite_this_stu;   /*!< is_invite_this_stu */
@property (nonatomic, copy) NSNumber *accept_apply_small_red_point; /*!< 是否显示小红点 */
